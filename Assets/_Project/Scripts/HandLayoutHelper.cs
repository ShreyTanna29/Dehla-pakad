...
public static class HandLayoutHelper
{
    ...
    public static float GetRowY(int row, bool twoRowHand)
    {
        ...
        // 2 Taash: top row +10, bottom row -10 vs previous 55/-55 for clearer row separation.
        return row == 0 ? 65f : -65f;
    }
    ...
    static float ResolveSpacing(float fitSpacing, float prefabCardWidth, int cardCount, int fullHandCount, bool is2Taash)
    {
        if (is2Taash)
        {
            ...
            {
                float resolved = ComputeHandSpacing(fitSpacing, prefabCardWidth, is2Taash);
            ...
            return ComputeHandSpacing(fitSpacing, prefabCardWidth, is2Taash);
        }
